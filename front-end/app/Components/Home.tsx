  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        fontSize: 19, fontWeight: 700, color: "#2c1f0e",
        marginBottom: 12, lineHeight: 1.25,
      }}>{title}</div>
      <div style={{
        fontFamily: "'Crimson Pro', Georgia, serif",
        fontSize: 15, color: "#7a6040", lineHeight: 1.72,
      }}>{desc}</div>
    </div>
  );
}
...
export default function Home({ onNavigate, onNavigateLogin }: { onNavigate?: () => void; onNavigateLogin?: () => void }) {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToPlatformStats(setStats);
    return () => {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes floatPaper { 0%,100% { transform: translateY(0) rotate(-1.5deg); } 50% { transform: translateY(-14px) rotate(1deg); } }
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 32, height: 32, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ width: 28, height: 28, border: "2px solid #8b6914", transform: "rotate(45deg)", position: "absolute" }} />
            <div style={{ fontSize: 13, fontFamily: "'Playfair Display', serif", fontWeight: 700, color: "#8b6914", position: "relative", zIndex: 1 }}>G</div>
          </div>
          <div>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 17, fontWeight: 800, color: "#2c1f0e", letterSpacing: "-0.01em", lineHeight: 1 }}>GenResearch</div>
            <div style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 9.5, color: "#8b6914", letterSpacing: "0.18em", textTransform: "uppercase" }}>Academic AI Platform</div>
          </div>
        </div>
      ...
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "130px 52px 90px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 79px, rgba(139,105,20,0.06) 80px)" }} />
        <div style={{ position: "absolute", right: "8%", top: "15%", width: 480, height: 480, borderRadius: "50%", border: "1px solid rgba(139,105,20,0.12)", pointerEvents: "none" }} />
        ...
        <div style={{ maxWidth: 660, position: "relative", zIndex: 10 }}>
          <div className="fade1" style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
            ...
            <span style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 11, fontWeight: 600, color: "#8b6914", letterSpacing: "0.18em", textTransform: "uppercase" }}>AI Research Platform · FYP 2025</span>
            <div style={{ height: 1, width: 40, background: "#8b6914" }} />
          </div>
          <h1 className="fade2" style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(42px, 6vw, 76px)", fontWeight: 900, color: "#2c1f0e", lineHeight: 1.05, letterSpacing: "-0.02em", marginBottom: 18 }}>
            The Intelligent<br />Research<br /><em style={{ color: "#8b6914", fontStyle: "italic" }}>Companion.</em>
          </h1>
          <div className="fade3" style={{ marginBottom: 14 }}>
            <span style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 22, color: "#7a6040", fontWeight: 300 }}>Generating </span>
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: "#8b6914", borderBottom: "2px solid #8b6914", paddingBottom: 1, minWidth: 10, display: "inline-block" }}>
              {typed}<span style={{ animation: "tickerCursor 1s step-end infinite", borderRight: "2.5px solid #8b6914", marginLeft: 2 }} />
            </span>
            <span style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 22, color: "#7a6040", fontWeight: 300 }}> — automatically.</span>
          </div>
          <p className="fade3" style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 17, color: "#7a6040", lineHeight: 1.8, maxWidth: 520, marginBottom: 44 }}>
            GenResearch is a multi-agent AI platform built for academics and students. Upload research papers, ask questions, and receive structured, cited outputs in seconds.
          </p>
          ...
          <div className="fade5">
            <div style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 11, color: "#8b6914", letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: 14 }}>Active Agents</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {[["Summarization", "#8b6914"], ["Literature Review", "#a0522d"], ["Citation", "#6b5c38"], ["Proposal Drafting", "#8b6914"]].map(([label, color]) => (
                <div key={label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", background: "rgba(139,105,20,0.07)", border: "1px solid rgba(139,105,20,0.2)", borderRadius: 2 }}>
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: color }} />
                  <span style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 13, color: "#5a3e20", fontWeight: 500 }}>{label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ padding: "60px 52px", background: "#2c1f0e", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 2 }}>
        {[
          { v: stats.total_users, s: "+", l: "Active Researchers" },
          { v: stats.total_papers, s: "+", l: "Papers Processed" },
          { v: stats.total_tasks_completed, s: "+", l: "Tasks Completed" },
          { v: stats.total_citations, s: "+", l: "Citations Generated" },
        ].map((st, i) => (
          <div key={i} className="stat-box" style={{ textAlign: "center", padding: "32px 20px", border: "1px solid rgba(255,255,255,0.07)", transition: "all .3s ease", cursor: "default" }}>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 46, fontWeight: 900, color: "#f5f0e8", lineHeight: 1, marginBottom: 8 }}>
              <Counter to={st.v} suffix={st.s} />
            </div>
            <div style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 12, color: "#8b6914", letterSpacing: "0.15em", textTransform: "uppercase" }}>{st.l}</div>
          </div>
        ))}
      </section>
      ...
      <section id="features" style={{ padding: "110px 52px", background: "#f5f0e8", scrollMarginTop: 64 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "flex-end", gap: 32, marginBottom: 64 }}>
            <div>
              <div style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 11, fontWeight: 600, color: "#8b6914", letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: 12 }}>Platform Architecture</div>
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(32px, 4.5vw, 52px)", fontWeight: 900, color: "#2c1f0e", letterSpacing: "-0.02em", lineHeight: 1.08 }}>Eight Modules,<br /><em style={{ color: "#8b6914" }}>One Purpose.</em></h2>
            </div>
            <p style={{ flex: 1, maxWidth: 400, fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 16, color: "#7a6040", lineHeight: 1.75, paddingBottom: 6 }}>Every component is purpose-built for academic research. Clean separation of concerns, independently testable, production-ready.</p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 52 }}>
            <div style={{ flex: 1, height: 1, background: "rgba(139,105,20,0.2)" }} />
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: "#8b6914" }}>✦</div>
            <div style={{ flex: 1, height: 1, background: "rgba(139,105,20,0.2)" }} />
          </div>
      ...
      <footer style={{ padding: "28px 52px", background: "#1e1408", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14 }}>
        <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 15, fontWeight: 800, color: "#c8971e" }}>GenResearch</span>
        <span style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 13, color: "rgba(200,151,30,0.45)" }}>Ali Ahmed · Kaleem-Ullah Abbasi · COMSATS WAH · FYP 2025</span>
        <div style={{ display: "flex", gap: 20 }}>
          {["Privacy", "Terms", "GitHub"].map(l => <a key={l} href="#" style={{ fontFamily: "'Crimson Pro', Georgia, serif", fontSize: 13, color: "rgba(200,151,30,0.45)", textDecoration: "none" }}>{l}</a>)}
        </div>
      </footer>
    </>
  );
