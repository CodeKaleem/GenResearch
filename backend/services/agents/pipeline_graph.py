...
def route_sufficiency(state: dict) -> str:
    ...
    # We can't really loop back to user input automatically. 
    # If it fails the retry cap, we just flag it and proceed to gap report.
    if decision == "retry":
...
def route_source_quality(state: dict) -> str:
    ...
    # Source quality eval is internally parallel and defaults to pass on parse failure.
    # We can implement a retry if the entire batch fails, but typically we proceed.
    # For now, just proceed to ingestion.
    return "ingestion"
...
def build_pipeline_graph() -> StateGraph:
    ...
    # Questionnaire to Sufficiency eval (Note: The frontend will pause here to collect answers.
    # When resumed with answers, it proceeds to sufficiency_eval).
    workflow.add_edge("questionnaire", "sufficiency_eval")
    ...
    # Fan out to Branch A & B
    workflow.add_edge("gap_report", "outline_plan")
    workflow.add_edge("gap_report", "source_gathering")
    
    # Branch B flow
    workflow.add_edge("source_gathering", "source_quality_eval")
    workflow.add_conditional_edges("source_quality_eval", route_source_quality, {
        "source_quality_eval": "source_quality_eval",
        "ingestion": "ingestion"
    })
    ...
    workflow.add_conditional_edges("citation_verify", route_citation_verify, {
        "citation_verify": "citation_verify",
        "merge_cd": "merge_cd"
    })
    
    workflow.add_conditional_edges("section_critic", route_section_critic, {
        "section_critic": "section_critic",
        "merge_cd": "merge_cd"
    })
    ...
    workflow.add_conditional_edges("final_qa", route_final_qa, {
        "final_qa": "final_qa",
        "output": "output"
    })
